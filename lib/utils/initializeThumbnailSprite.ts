...
function initializeThumbnailSprite(player: videojs.Player, options: TS.Options): void {
  ...
  for (let i = 0 ; i < sprites.length - 1 ; i++) {
    ...
    if (e2Start < e1End) {
      ...
      console.warn(`Provided thumbnail sprites have overlapping sections among them: [${secondToTimeString(overlapStart)} ~ ${secondToTimeString(overlapEnd)}] Preview images may not match the corresponding point of time correctly.`);
    }
  }
